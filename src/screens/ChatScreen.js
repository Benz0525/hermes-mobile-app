...
export default function ChatScreen({ route, navigation }) {
    ...
    try {
      ...
      const msgs = raw
        ...
        .map(m => ({
          ...
          timestamp: m.timestamp ? (m.timestamp > 1e11 ? m.timestamp : m.timestamp * 1000) : (m.created_at ? m.created_at * 1000 : Date.now()),
          _fromCloud: true,
        }));
      setMessages(msgs);
  ...
  const typewriterTick = useCallback(() => {
    ...
    // v5.3.3: reasoning 也走 35ms 节流，一次吐完蓄水池，避免每个 token 都 setState 掉帧
  ...
  const sendToAI = async (userMsg) => {
    ...
    abortRef.current = sendMessageStream(
      ...
      (chunk) => {
        ...
        // v5.3.3: reasoning 不再立即 flush，囤入蓄水池由 typewriter tick (35ms) 节流消费，避免每 token setState 掉帧
        if (chunk.reasoning) {
      ...
      async () => {
        ...
        // v5.3.3: 把 pendingRef 里剩余的 text 和 reasoning 一次性追加
        const remainingText = pendingRef.current.text;
        const remainingReasoning = pendingRef.current.reasoning;
        pendingRef.current.text = '';
        pendingRef.current.reasoning = '';
        if (remainingText || remainingReasoning) {
          const msgs = [...messagesRef.current];
          const last = msgs[msgs.length - 1];
          if (last && last.role === 'hermes') {
            if (remainingText) last.text = (last.text || '') + remainingText;
            if (remainingReasoning) last.reasoning = (last.reasoning || '') + remainingReasoning;
            messagesRef.current = msgs;
            setMessages([...msgs]);
          }
        }
      ...
      (error) => {
        ...
        // v5.3.3: 出错时 text 和 reasoning 残余都补上，避免丢字
        const remainingText = pendingRef.current.text;
        const remainingReasoning = pendingRef.current.reasoning;
        pendingRef.current = { text: '', reasoning: '', toolCalls: [] };
        setIsStreaming(false);
      ...
      // v5.3.3: meta 调试信息回调 — 新建消息对象替换 last，避免 mutate 导致 FlatList/memo 不重渲染
      (meta) => {
        const msgs = [...messagesRef.current];
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ref={flatListRef}
        data={messages}
        extraData={isStreaming ? (messages[messages.length-1]) : messages.length}
